...
/** 🔥 heat bar: score/100 scaled to 5 dots */
export function heatBar(score: number): string {
  const filled = Math.min(5, Math.max(0, Math.round((score / 100) * 5)));
  return '\uD83D\uDD25'.repeat(filled) + '\u2B1B'.repeat(5 - filled);
}

/** ▓▓▓▓░░ multiplier bar — log scale so 10x = full bar */
export function multBar(mult: number): string {
  const filled = Math.min(10, Math.max(0, Math.round(Math.log2(Math.max(1, mult)) * 3)));
  return '\u2593'.repeat(filled) + '\u2591'.repeat(10 - filled);
}

/** Quality tier badge based on algo score */
export function qualityTier(score: number): string {
...
export function formatCallMessage(token: ScoredToken, holders?: HolderAnalysis | null): string {
  ...
  const tier  = qualityTier(token.score);

  let msg = `\uD83C\uDFAF <b>$${p.baseToken.symbol}</b>`;
  if (tier) msg += `  ${tier}`;
  msg += `\n<b>${p.baseToken.name}</b>\n`;
  msg += `<code>${ca}</code>\n\n`;
  ...
  msg += '\n';
  msg += `\uD83D\uDCC8 5m ${fmtPct(pc5m)}  1h ${fmtPct(pc1h)}  6h ${fmtPct(pc6h)}\n`;
  msg += `\uD83D\uDCCA <b>Vol/1h:</b> ${fmtVol(vol1h)}  \uD83D\uDCA7 <b>Liq:</b> ${fmtVol(liq)}\n`;
  msg += `\uD83D\uDD04 ${buys}\u2191 / ${sells}\u2193  (${buys + sells > 0 ? ((buys / (buys + sells)) * 100).toFixed(0) : 0}% buys)\n\n`;
  ...
  if (holders) {
    msg += `  Bundle: ${riskBadge(holders.riskLevel)}  Top10: ${holders.top10Pct.toFixed(1)}%\n`;
...
export function formatUpdateMessage(
  ...
  let msg = `\uD83D\uDCCA <b>$${call.symbol} \u2014 ${milestone}</b>\n\n`;
...
export function formatStatsMessage(stats: {
  totalCalls: number; calls24h: number;
  bestMultiplier: number; bestSymbol: string;
  proSubscribers: number; activeFastTracks: number;
}): string {
  return (
    `\uD83D\uDCCA <b>Kabal Radar \u2014 Stats</b>\n\n` +
    `\uD83D\uDCDE Total calls: <b>${stats.totalCalls}</b>\n` +
    `\uD83D\uDD52 Last 24h: <b>${stats.calls24h}</b>\n` +
    `\uD83C\uDFC6 Best: <b>$${stats.bestSymbol}</b> @ <b>${stats.bestMultiplier.toFixed(1)}x</b>\n` +
