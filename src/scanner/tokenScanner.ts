...
import { getPumpNewMints } from './pumpFun';
...
export async function scanTokens(): Promise<void> {
  ...
  try {
    ...
    // ─ Source 1+2: DexScreener search
    for (const q of ['pump.fun', 'pumpfun']) { allPairs.push(...await searchPairs(q)); await sleep(400); }

    // ─ Source 3: DexScreener profiles
    const profiles = await getLatestTokenProfiles();
    const profAddrs = profiles.filter(p => p.chainId === 'solana').slice(0, 20).map(p => p.tokenAddress);
    if (profAddrs.length) allPairs.push(...await getTokensByAddresses(profAddrs));

    // ─ Source 4+5: Birdeye
    const beTrending = await getTrendingByVolumeMomentum(50);
    if (beTrending.length) allPairs.push(...await getTokensByAddresses(beTrending.map(t => t.address)));
    const beNew = await getNewListings(30);
    if (beNew.length) allPairs.push(...await getTokensByAddresses(beNew.map(t => t.address)));

    // ─ Source 6+7: GeckoTerminal (no key)
    const geckoT = await getGeckoTrendingAddresses(20);
    if (geckoT.length) { allPairs.push(...await getTokensByAddresses(geckoT)); await sleep(300); }
    const geckoN = await getGeckoNewPoolAddresses(20);
    if (geckoN.length) allPairs.push(...await getTokensByAddresses(geckoN));

    // ─ Source 8: Pump.fun direct (catches tokens before DexScreener)
    const pumpMints = await getPumpNewMints(30);
    if (pumpMints.length) allPairs.push(...await getTokensByAddresses(pumpMints));

    // Deduplicate
    const seen   = new Set<string>();
    const unique = allPairs.filter(p => { if (seen.has(p.baseToken.address)) return false; seen.add(p.baseToken.address); return true; });
    console.log(`[Scanner] ${unique.length} unique from ${allPairs.length} raw`);

    // Batch mint info
    const mintInfoMap = new Map<string, MintInfo | null>();
    await Promise.all(unique.map(async p => { mintInfoMap.set(p.baseToken.address, await getMintInfo(p.baseToken.address)); }));
    ...
    for (const token of scored) {
      ...
      // Rugcheck pre-call filter (only called for tokens that pass all other filters)
      const rug = await getRugcheckReport(addr);
      if (rug && rug.scoreNormalised > 70) {
        console.log(`[Scanner] Skip $${token.pair.baseToken.symbol} \u2014 rugcheck ${rug.scoreNormalised}/100`);
        continue;
      }

      const holders = await analyzeHolders(addr);
      if (config.scanner.skipHighRiskBundles && holders?.riskLevel === 'HIGH') continue;

      console.log(`[Scanner] Calling $${token.pair.baseToken.symbol} (score: ${token.score}, rug: ${rug?.scoreNormalised ?? '?'})`);
...
export async function manualCall(contractAddress: string): Promise<{ ok: boolean; msg: string }> {
  try {
    ...
    await postCall(token, 'algo', await analyzeHolders(contractAddress) ?? undefined);
...
async function processFastTrack(ft: any): Promise<void> {
  try {
    ...
    const token: ScoredToken = scored[0] ?? { pair: pairs[0], score: 0, reasons: ['Fast-Track'], warnings: [], isCto: false, isDexscreenerPaid: false };
    await postCall(token, 'fasttrack', await analyzeHolders(ft.contract_address) ?? undefined);
  } catch (err: any) { console.error('[Scanner] Fast-track error:', err.message); }
}

function sleep(ms: number) { return new Promise(r => setTimeout(r, ms)); }

export function startScanner(): void {
  console.log(`[Scanner] Starting \u2014 ${config.scanner.intervalMinutes}min interval, 8 sources`);
  scanTokens();
